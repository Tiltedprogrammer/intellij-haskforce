...
final class StackManager
  ...
  override def getAffectedExternalProjectPath(
    ...
    project: Project
  ): String = { // TODO: Is this right?
    val stackYamlPath = HaskellBuildSettings.getInstance(project).getStackFile
    if (stackYamlPath == changedFileOrDirPath) return stackYamlPath
    null
  }
}

object StackManager {
  val PROJECT_SYSTEM_ID = new ProjectSystemId("haskforce-stack", "Stack")
}
